...
loadPath = '../data/processed/ZePSI-E-013022-ESEEM.mat';
load(loadPath)
nMeas = numel(y);
for ii = 1:nMeas
...
integWidth = 10;
iMax = 196;
deltaTau = findparampulsespel(Param{1}, 'd30');  % ns
nMeas = numel(y);
...
xeseem = 180 + (0:deltaTau:(nTau - 1)*deltaTau);
for ii = 1:nMeas
...
for ii = 1 %:numel(I_BEST)
    plot(xbeta*180/pi/1.05, (-1)^ii*ybeta(:, ii)/max(abs(ybeta(:, ii))), 'o-', 'DisplayName', string(I_BEST(ii)))
    hold on
end
yline(0, 'HandleVisibility', 'off')
legend()
