...
for ii = 1:nMeas
    [yout{ii}, bestPhase(ii)] = correctphase(yst0{ii}, 'integral');
    maxval(ii) = max(real(yout{ii}));
    imagval(ii) = trapz(abs(imag(yout{ii})));
end

ifit = 2:25;  % [1:11, 15:20];
...
for ii = 1:nMeas
    for itau = 1:size(yel0{ii}, 1)
        ...
        yed1{ii}(itau, :) = shiftphase(yed0{ii}(itau, :), fitPhase2(ii));
        yst1{ii} = shiftphase(yst0{ii}, fitPhase2(ii));
    end
end
...
%%
II = 25;
correctphaseslider(xst, yst0{II}, 0);
