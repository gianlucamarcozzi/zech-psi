...
loadPath = '../data/processed/ZePSI-E-013002-Nut1.mat';
load(loadPath)
...
clf
tiledlayout("flow", "TileSpacing", "compact", "Padding", "compact")
w0 = [0.01*ones(1, 18), 4, 5, 7, 10, 17, 26, 34, 38]*1e-3;
for ii = 1:nMeas
...
nexttile
for ii = 1:nMeas
    freq(ii) = abs(pfit{ii}(2));
end
% errorbar(1:nMeas, freq, pci(:, 2, 1) - freq', pci(:, 2, 2) - freq', 'o')
errorbar(1:nMeas, freq/0.0398, ...
    (abs(pci(:, 2, 1)) - freq')/0.0398, ...
    (abs(pci(:, 2, 2)) - freq')/0.0398, 'o')
xlim(setaxlim(1:nMeas, 1.1))
ylim([-0.25, 1.1])
yline(1)

for ii = 1:nMeas
    Param{ii}.turningAngle = freq(ii)/0.0398*pi;
end
