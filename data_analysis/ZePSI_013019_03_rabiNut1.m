...
loadPath = '../data/processed/ZePSI-E-013019-Nut1.mat';
load(loadPath)
nMeas = numel(x);
...
%%
% ------------------------------------------------------------------------
FREQ_PI_PULSE = 36.91;  % MHz, taken from the rabiNut2.m data analysis file
% ------------------------------------------------------------------------
...
% Turning angle is equal to length of the pulse (p1 = 12ns) divided by
% length of a pi pulse at the given mpfu channel amplitude (times pi to
% make it an angle).
loadEseemPath = append(loadPath(1:end-9), '-ESEEM.mat');  % MODIFY
LoadEseem = load(loadEseemPath);
turningAngle = zeros(1, nMeas);
for ii = 1:nMeas
    turningAngle(ii) = 12/piPerf(ii)*pi;
...
% save('../data/processed/ZePSI_013019_mpfuXch.mat', 'sigfun', 'pfitsig')
