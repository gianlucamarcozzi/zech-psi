...
loadPath = '../data/processed/ZePSI-E-013035-Nut1.mat';
load(loadPath)
nMeas = numel(x);
...
% w0 = (linspace(sqrt(4), sqrt(37), nMeas)).^2*1e-3;
w0 = (linspace(sqrt(2), sqrt(20), nMeas)).^2*1e-3;
for ii = 1:nMeas
    p0 = [100, w0(ii)];
...
%%
% ------------------------------------------------------------------------
FREQ_PI_PULSE = 22.02;  % MHz, taken from the rabiNut2.m data analysis file
RABI_LENGTH_PI_PULSE = 1/2/FREQ_PI_PULSE*1e3;
% ------------------------------------------------------------------------
...
pulseLength = findparampulsespel(Param{1}, 'p1 ');
for ii = 1:nMeas
    turningAngle(ii) = RABI_LENGTH_PI_PULSE/piPerf(ii)*pi; % *(pulseLength/RABI_LENGTH_PI_PULSE);
